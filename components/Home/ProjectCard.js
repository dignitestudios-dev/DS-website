...
const ProjectCard = ({ title, description, image, projectLink }) => {
  ...
  return (
    <Link
      ...
      className="group w-full h-auto  p-3 lg:p-0 flex flex-col items-start gap-2 lg:mb-12 relative">
      <picture>
        <source srcset={image} media="(orientation: portrait)" />
        <img src={image} alt={title} className="w-full  rounded-xl md:rounded-3xl md:h-[220px] lg:h-[340px]  hover:brightness-80 transition-all duration-700" />
      </picture>
      ...
      <div className="flex flex-col justify-start my-1 items-start md:px-2">
        ...
        <div className="w-full flex justify-between items-center pr-1">
          ...
          <button className="">
            <img src={"/case-study-arrow.webp"} alt="" className="w-4 lg:w-5 " />
          </button>
        </div>
        <p

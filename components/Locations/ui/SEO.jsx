...
const SEO = ({ header, para, locations: customLocations }) => {
  ...
  return (
    <div className="w-full grid grid-cols-1 justify-start items-start py-20 lg:py-32">
      <div className="flex relative z-20 w-[90%] md:w-[70%] flex-col text-center mx-auto items-center mb-12 px-4">
        <h2 className="text-4xl leading-tight tracking-tighter md:text-7xl font-bold">
          {header || "Specific Coverage "}
        </h2>
        <p className="text-base md:text-lg my-6 dark:text-white text-[#0C0C0C] max-w-7xl">
          {para ||
      ...
      <div
        ref={ref}
        className="relative md:block hidden -my-24 w-full max-w-7xl mx-auto px-4"
      >
        <div className="relative w-full" style={{ minHeight: "500px" }}>
          ...
          >
            {dimensions.width > 0 &&
              locationsToUse.map((location, index) => {
                ...
                const cardY =
                  (location.cardPercent.y / 100) * dimensions.height;
          ...
          {locationsToUse.map((location, index) => (
            ...
            >
              <div
                className={` text-nowrap inline-block bg-[#F15C20] text-white px-3 py-1 rounded text-sm font-bold mb-3`}
              >
                {location.title}
              </div>
        ...
        <div className="flex flex-col gap-8 max-w-xl mx-auto">
          {locationsToUse.map((location, index) => (
            <motion.div
              ...
              className="bg-white border-l-4 border-[#F15C20] rounded-2xl p-6 shadow-sm border border-gray-100"
            >
              <span className="inline-block bg-[#F15C20]/10 text-[#F15C20] px-3 py-1 rounded-full text-xs font-bold mb-4">
                {location.title}
              </span>

              <h3 className="font-bold text-xl leading-tight mb-2 text-black">
                {location.heading}
              </h3>
